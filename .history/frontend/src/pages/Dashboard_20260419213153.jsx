      ...
      <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16}}>
        ...
        <div style={{background: '#fff', border: '1px solid #e5e4df', borderRadius: 12, padding: 20}}>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: 10}}>
            {[
              {label: 'Applied', value: stats.applied, total: stats.total, color: '#378ADD'},
              {label: 'Interview', value: stats.interview, total: stats.total, color: '#EF9F27' },
              {label: 'Offer', value: stats.offer, total: stats.total, color: '#63922' },
              {label: 'Rejected', value: stats.rejected, total: stats.total, color: '#E24B4A' },
            ].map(({ label, value, total, color }) => (
              <div key={label} style={{display: 'flex', alignItems: 'center', gap: 8}}>
                <span style={{fontSize: 12, color: '#6b6b67', width: 60}}>{label}</span>
                <div style={{flex: 1, background: '#f9f9f8', borderRadius: 4, height: 10}} />
                </div>
